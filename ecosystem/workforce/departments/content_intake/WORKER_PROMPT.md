...
```
STEP 0 — Local-First Check (ANTIGRAVITY — BEFORE CREATING TICKET):
  → LightRAG: rag.hybrid_query("<source>", mode="mix")
  → brain/knowledge/INDEX.md
  FOUND (>=0.7) -> CEO, why change? → if NO → STOP
...
STEP 3.5 — content-analyst-agent (REPO/PLUGIN path after PASS):
   ...
   6 fruits:
     ...
     5. "Does this OmniClaw domain have an agent/dept solution?"  <- gap detection
     6. "If you have any questions, is it right to remove the agent/dept?"  <- gap detection
   Output: _CIV_ANALYSIS.md with fields:
