...
RULE OD-01: ORG DESIGN IS EVIDENCE-BASED
  ...
  Proposals without evidence → returned by org-architect-agent.
...
RULE OD-03: ALL ORG CHANGES NEED CEO APPROVAL
  ...
  → Proposal to CSO → then to CEO for final approval.
...
RULE OD-07: DEPT BUILD = FULL PACKAGE
  ...
  - memory file (brain/knowledge/org/<dept>.md)
...
RULE OD-08: COORDINATION WITH OTHER DEPTS
  ...
  → New dept: notify all affected C-Suite + update org_chart.yaml
  → New agent: coordinate with HR (onboard) + Registry (register skills)
  → New skill assignment: coordinate with Registry (validate skill exists)
...
**This agent is the architectural intelligence of OmniClaw OS**
**Responsibilities:**
...
**Must load at boot:**
- `brain/knowledge/org/od_learning.md`
- `corp/org_chart.yaml` — current full org structure
- `ecosystem/workforce/departments/od_learning/MANAGER_PROMPT.md`
- Latest retro: `shared-context/brain/corp/proposals/RETRO_*.md` (most recent)
**Skills:**
- `reasoning_engine` — org design decisions
- `cognitive_reflector` — pattern synthesis from learning loop
- `context_manager` — full org-wide context management
- `knowledge_enricher` — research organizational best practices
**Key principle:** Propose systemic solutions, not local fixes.

---

### dept-builder-agent
**Role:** Full-cycle department construction — from proposal to deployed
...
- SKILL: `reasoning_engine` — dept design decisions
- SKILL: `context_manager` — multi-file simultaneous creation
- `corp/org_chart.yaml` — current structure reference
- `skills/SKILL_SPEC.md` — for any new skills needed
- `corp/memory/MEMORY_SPEC.md` — for memory file format
- Template structures from existing `ecosystem/workforce/departments/` examples
**Skills:**
- `reasoning_engine` — architecture and design
- `context_manager` — multi-file orchestration
**Build checklist:**
  - [ ] rules.md (domain rules + all agent specs with skills + tools)
  - [ ] MANAGER_PROMPT.md (activation + daily brief + KPIs)
  - [ ] brain/brain/knowledge/org/<dept>.md
...
- TYPE 2 (Skill): check SKILL_REGISTRY.json → patch agent rules.md or request new skill from skill-creator-agent (Registry)
- TYPE 3 (Rule): draft rule → rule-builder-agent validates → C-Suite approves → deploy
- TYPE 4 (Dept Drop): write to brain/brain/knowledge/org/<dept>.md rolling memory
...
- SKILL: `knowledge_enricher` — search SKILL_REGISTRY for best skill match
- SKILL: `reasoning_engine` — skill-agent fit assessment
- SKILL: `cognitive_evolver` — agent capability evolution assessment
- SKILL: `cosmic_memory` — check if enrichment is worth permanent retention
- `corp/sops/ENRICHMENT_SOP.md` — enrich type definitions + formats
- `shared-context/SKILL_REGISTRY.json` — available skills
- Agent's current memory + rules spec
- `corp/knowledge_feeds/od_learning/new_knowledge.md` — incoming enrichment triggers
**Skills:**
- `knowledge_enricher` — skill discovery and matching
- `reasoning_engine` — training strategy
- `cognitive_evolver` — agent capability evolution assessment
- `cosmic_memory` — identifies which enrichments deserve permanent retention
**Upgrade output:**
...
**Key SOP:** brain/corp/sops/ENRICHMENT_SOP.md — read before every enrichment cycle.
...
- SKILL: `cognitive_reflector` — cross-dept pattern analysis
- SKILL: `reasoning_engine` — root cause analysis
- SKILL: `knowledge_enricher` — aggregate daily brief data
- All 20 dept daily_briefs from `shared-context/brain/corp/daily_briefs/`
- `brain/shared-context/kpi_targets.json`
**Skills:**
- `cognitive_reflector` — ALWAYS. Pattern detection across departments.
- `reasoning_engine` — causal analysis
- `knowledge_enricher` — data aggregation from briefs
**Key patterns to detect:**
- Same escalation appearing in 2+ depts → systemic issue
- Agent with >3 consecutive failures → training gap
- Task type with no skilled agent → skill gap / new agent needed
- Dept handoff delay >1 cycle → INTER_DEPT_SOP failure
...
- SKILL: `knowledge_enricher` — extract insights from retro docs
- SKILL: `cosmic_memory` — permanent learning persistence
- SKILL: `cognitive_reflector` — insight synthesis
- All RETRO_*.md files from `shared-context/brain/corp/proposals/`
**Skills:**
- `knowledge_enricher` — PRIMARY TOOL. Retro extraction.
- `cosmic_memory` — permanent retention of key learnings
- `cognitive_reflector` — synthesis and pattern extraction
**Output format:**
```markdown
## Learning Entry — [DATE]
...
Cosmic: [YES/NO — worth permanent retention]
```