...
def build_narrative_for_lightrag():
    ...
    narrative_lines = []
    narrative_lines.append("TITLE: OmniClaw MASTER KNOWLEDGE GRAPH NARRATIVE")
    narrative_lines.append(f"DATE: {datetime.now().strftime('%Y-%m-%d')}")
    narrative_lines.append("DESCRIPTION: This document describes the exact architecture and entities operating within the OmniClaw. It defines what systems live in the Local Core vs Remote Ecosystem.")
    ...
    for block in blocks[1:]:
        ...
        narrative = f"The entity '{e_name}' (ID: {e_id}) is a {e_type} component located at '{e_path}'. It belongs to the {e_cat} architectural branch of the OmniClaw. "

        if e_cat == "LOCAL_CORE":
            narrative += f"Because '{e_name}' is in LOCAL_CORE, it is an isolated backend system, agent, or logic core that operates securely inside the main OmniClaw engine."
