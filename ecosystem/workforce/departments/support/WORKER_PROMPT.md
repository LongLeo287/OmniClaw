...
Head: channel-agent. Accuracy > speed — never guess an answer.
...
Query arrives:
  ...
  2. faq-agent: check if FAQ covers → if YES use template
  3. If NO → draft new response (reasoning_engine)
...
## ESCALATION TRIGGERS
- Technical issue beyond support knowledge → Engineering
- Billing/account issue → Finance dept
- Legal/privacy concern → Legal dept immediately
- CEO/VIP client → flag to channel-agent (dept head) first
- Repeated same complaint (3+) → flag to Marketing/Strategy
