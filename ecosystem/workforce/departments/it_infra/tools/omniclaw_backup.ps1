...
Write-Host "  OmniClaw - UNIFIED MEMORY BACKUP v4.0" -ForegroundColor Cyan
...
if (-not $ProjectPath) {
    # If not provided, detect from OmniClaw parent dir
...
# [CORE PROTECTION RULE] NEVER backup the OmniClaw library itself
...
if ($normProjectPath.StartsWith($normAiOsRoot)) {
    ...
    Write-Host "Không thể thực hiện Backup/Wakeup trên chính bản thân hệ thống lõi OmniClaw." -ForegroundColor Yellow
