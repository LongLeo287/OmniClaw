...
if not exist "%OMNICLAW_ROOT%\.env" (
    echo  [ERROR] .env file not found at %OMNICLAW_ROOT%\.env
    exit /b 1
)
...
start "AOS-API-Bridge" /min cmd /c "node "%OMNICLAW_ROOT%\api\server.js" > "%LOGS_DIR%\api-bridge.log" 2>&1"
...
start "MCP-omniclaw-workspace" /min cmd /c "node "%OMNICLAW_ROOT%\mcp\servers\omniclaw-workspace\index.js" 2> "%LOGS_DIR%\mcp-omniclaw-workspace.log""
timeout /t 1 /nobreak > nul
echo      OK "” stdio (Claude Code MCP)
...
start "MCP-skill-registry" /min cmd /c "node "%OMNICLAW_ROOT%\mcp\servers\skill-registry\index.js" 2> "%LOGS_DIR%\mcp-skill-registry.log""
timeout /t 1 /nobreak > nul
echo      OK "” stdio (Claude Code MCP)
...
start "MCP-corp-data" /min cmd /c "node "%OMNICLAW_ROOT%\mcp\servers\corp-data\index.js" 2> "%LOGS_DIR%\mcp-corp-data.log""
timeout /t 1 /nobreak > nul
echo      OK "” stdio (Claude Code MCP)
