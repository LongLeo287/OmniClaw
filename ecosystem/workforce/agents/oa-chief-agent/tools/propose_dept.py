#!/usr/bin/env python3
"""
propose_dept.py — OmniClaw Corp V3.1 Autonomous Department Proposal & Activation Engine
Path: system/ops/scripts/propose_dept.py

Antigravity scans org_chart → tìm gaps → tạo proposals → CEO approve → auto-execute.

Usage:
  python system/ops/scripts/propose_dept.py --scan           # [Removed legacy comment]
  python system/ops/scripts/propose_dept.py --propose        # [Removed legacy comment]
  python system/ops/scripts/propose_dept.py --approve <id>   # [Removed legacy comment]
  python system/ops/scripts/propose_dept.py --approve-all    # [Removed legacy comment]
  python system/ops/scripts/propose_dept.py --list           # [Removed legacy comment]
"""
...
ROOT = Path(__file__).parent.parent.parent.parent  # system/ops/scripts -> OmniClaw root
...
def cmd_scan():
    ...
    print("\nOmniClaw Corp — Department Gap Analysis")
    print("=" * 50)
    gaps = scan_gaps()
...
# OmniClaw Corp V3.1 | Created: {today()}
...
You report to {head} and operate under OmniClaw governance.
    ...
    if agent_exists:
        ...
        try:
            ...
            if f"Department: {dept}" not in content:
                ...
                elif re.search(r'\| \*\*Department\*\*.*\|', content):
                    content = re.sub(
                        r'(\| \*\*Department\*\*\s*\|).*?(\|)',
...
def main():
    ...
    if not args or "--help" in args:
        print("""
propose_dept.py — OmniClaw Corp Department Proposal & Activation Engine

Commands:
  --scan              Analyze tất cả 21 phòng ban, tìm gaps
