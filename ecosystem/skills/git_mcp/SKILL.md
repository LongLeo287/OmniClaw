...
git_read_file(repo_path="$OMNICLAW_ROOT", file_path="skills/context7/SKILL.md", ref="HEAD")
```
...
// Already in claude_desktop_config.json:
{
  "mcpServers": {
    "git": {
      "command": "uvx",
      "args": ["mcp-server-git", "--repository", "$OMNICLAW_ROOT"]
    }
  }
}
```
