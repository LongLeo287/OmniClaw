...
> **Status Indicators:**
> - 👁️ = Reviewed/Surveyed
> - 📚 = Reference & Study (do not integrate tool — extract knowledge/patterns to brain/knowledge/notes/)
> - 🔖 = Keep (DEFER — use later, specify reason)
> - ✅ = Active (monitor versions)
> - ⚡ = Integrating
> - ❌ = Completely remove (REJECT — no usage, no learning — specify reason)
> - ⏸️ = Unread (need to run repo-evaluation.md)
...
| `AntigravityManager` | 🔖 DEFER → Phase 5 | Tier 2 | Check overlap | **Verdict:** DEFER pending dedup check với antigravity-manager. Có thể là cùng 1 repo. Pending confirmation. |
| `antigravity-manager` | 🔖 DEFER → Phase 5 | Tier 2 | Check overlap | **Verdict:** DEFER. Dedup check với AntigravityManager. If overlap -> keep 1, REJECT the other. |
| `AstrBot` | 🔖 DEFER → Phase 5 | Tier 2 | Check vs nullclaw | **Verdict:** DEFER. Bot framework — needs comparison with nullclaw (current Telegram bot). If nullclaw suffices -> REJECT. |
...
| `OpenSandbox` | ❌ REJECT | Tier 3 | ❌ Duplicate e2b | **Verdict:** REJECT. e2b được maintain is better, có team, có docs đầy đủ hơn OpenSandbox. |
...
| `claude-bug-bounty` | ❌ REJECT | Tier 3 | — | **Verdict:** REJECT. This is flow/methodology, not an integrable tool. Reference docs instead of cloning into system. |
| `trufflehog` | 🔖 DEFER → Phase 3 | Tier 2 | Safe — complement zeroleaks | **Verdict:** DEFER. Secret scanning deep scan (Git history). Complement zeroleaks. Dept 10 (Security) integrate vào strix-scan pipeline. |
...
| `ralph-claude-code` | 👁️ | Tier 2 | Safe | **Verdict:** Read. Ralph Claude Code tools — reference for alternative implementations. |
...
| Repo | Lý do REJECT |
|------|-------------|
| `graphrag` | Duplicate LightRAG, tốn API cost cloud, LightRAG local is better |
| `claude-mem` | Duplicate mem0, chỉ hỗ trợ Claude, mem0 cross-platform |
| `openrag` | nexusrag is better, nay cả nexusrag cũng bị thay bởi LightRAG |
...
| `OpenSandbox` | Duplicate e2b, e2b maintained is better |
