# OmniClaw Corp — Remote Control System HANDOFF
...
nullclaw adalah gateway tự động hóa chính của OmniClaw Corp — kết nối Telegram của CEO (@LongLeo) với toàn bộ hệ thống AI agents. CEO điều khiển mọi thứ qua chat.
...
- [ ] **TG-002** Thêm OmniClaw Corp system prompt chi tiết hơn vào config
- [ ] **TG-003** Auto-start bot khi Windows boot (Task Scheduler)

### 🟡 MEDIUM — Sprint này

- [ ] **omniclaw-004** AstrBot WebUI config: thêm Telegram token qua http://localhost:6185/
- [ ] **omniclaw-005** Plugin `astrbot_plugin_aios_corp` — test /clawtask command
- [ ] **omniclaw-006** Sync ClawTask → JSON file để plugin đọc được
