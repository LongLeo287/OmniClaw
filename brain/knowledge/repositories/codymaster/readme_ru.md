...
**34 навыка · 11 команд · 1 плагин · 7+ платформ · 6 языков**
...
CodyMaster — это не просто "очередной набор навыков для AI". Это **10+ лет опыта управления продуктами + 6 месяцев проверенного в боях vibe coding**, воплощенные в 34 взаимосвязанных навыках, которые работают как **единая интегрированная система**.
...
- **`cm-continuity`** — Рабочая память между сессиями. AI помнит, что пошло не так, и никогда не повторяет ту же ошибку
- **`cm-skill-mastery`** — Не знает, как что-то сделать? Он **находит нужный навык автоматически** и самосовершенствуется
- **`cm-deep-search`** — Потерялись в кодовой базе из 200+ файлов? Семантический поиск по всему за считанные секунды
...
Больше никаких догадок. Отслеживайте каждую задачу, каждого агента, каждое развертывание на Kanban-доске в реальном времени. Прогресс пайплайна, трекер токенов, лог событий — всё на одном экране.
...
|---|---|---|
| **Интеграция** | Каждый навык автономен, нет общего контекста | 34 навыка, которые объединяются в цепочки, разделяют память и взаимодействуют |
...
### NPM (Универсальная установка)
```bash
npm install -g codymaster
codymaster
```
*Автоматически обнаруживает и устанавливает для Claude Code, Cursor, Gemini и других.*
