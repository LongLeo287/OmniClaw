...
owner: OHD
tags: [auto-healed]
healed_at: 2026-04-02T20:31:08.919942
---
...
| Status | Meaning |
|--------|---------|
| ✅ CLONE | High value, needs local code/files in OmniClaw |
| 📚 LEARN-ONLY | Extract knowledge/patterns, no need to clone |
| 🗂️ STUDY | Deep study required before deciding |
| ⏭️ SKIP | Low relevance to current OmniClaw phase |
...
- **Why CLONE:** Its agent templates, PRD generators, and workflow structures directly align with OmniClaw architecture. Can adopt "Party Mode" multi-agent collaboration concept.
- **Action:** Clone into `D:\APP\OmniClaw\knowledge\bmad\` and extract: agent personas, workflow templates, PRD structures.
...
- **Why LEARN-ONLY:** Pure Python/LangChain code, not directly usable. But the architectural *patterns* are goldmine for OmniClaw agent design.
...
- **Why CLONE:** This is exactly what OmniClaw already builds. We can adopt its hooks system (auto-lint after write), skills templates, and slash commands.
- **Action:** Clone into `D:\APP\OmniClaw\knowledge\everything-claude-code\` — extract hooks, skills, command patterns.
...
- **Why SKIP:** Vague relevance; not enough signal for current OmniClaw phase.
...
- **Why STUDY:** OmniClaw Phase 10 needs semantic search. LightRAG could power our local knowledge retrieval. Requires 32B+ LLM for entity extraction — need to validate local hardware fit.
...
- **Why STUDY:** The "Agent Kernel" concept mirrors OmniClaw philosophy. MCP integration and plugin architecture are directly relevant. But requires Python environment and external LLM.
...
- **Why SKIP:** Research/benchmark tool, not an infrastructure tool for OmniClaw.
...
- **Why SKIP:** Low direct relevance to current OmniClaw development phase.
...
- **Why SKIP:** Smart Bookmark Manager extensions. Relevant to the extension project, not to OmniClaw core.
...
- **Why CLONE:** OmniClaw already has `visual_excellence` skill. This extends it. Install as skill plugin.
- **Action:** Clone into `D:\APP\OmniClaw\skills\ui_ux_pro_max\`.

### ⏭️ SKIP — gaia-ui, KawaiiLogos
- **Why SKIP:** Static UI components/design assets, not directly useful for OmniClaw logic.
