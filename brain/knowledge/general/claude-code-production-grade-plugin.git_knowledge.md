---
id: claude-code-production-grade-plugin.git-knowledge
type: document
owner: OHD
tags: [auto-healed]
healed_at: 2026-04-02T20:19:02.892662
---

# KNOWLEDGE EXTRACT: claude-code-production-grade-plugin.git
> **Extracted on:** 2026-03-30 13:17:44
> **Source:** claude-code-production-grade-plugin.git
...
**Inputs:**
- **`brain/knowledge/docs_legacy/architecture/`**, **`api/`** — ADRs, API contracts (OpenAPI/AsyncAPI), data models, sequence diagrams, architectural decisions, technology choices
...
**Goal:** Verify that the implementation faithfully follows the architectural decisions documented in `brain/knowledge/docs_legacy/architecture/`. Flag every deviation.

**Inputs to read:**
- `brain/knowledge/docs_legacy/architecture/` ADRs (every Architecture Decision Record)
- `brain/knowledge/docs_legacy/architecture/` system architecture diagrams, service boundaries, communication patterns
...
- A table listing every ADR from `brain/knowledge/docs_legacy/architecture/` and its conformance status (Conformant / Partial / Violated)
...
- `brain/knowledge/docs_legacy/architecture/` NFRs (latency targets, throughput requirements)
...
| 2 | Flagging code without reading the ADR that justified it | The "violation" may be an intentional, documented trade-off | Always cross-reference `brain/knowledge/docs_legacy/architecture/` ADRs before flagging an architectural concern |
...
- [ ] `architecture-conformance.md` audits every ADR in `brain/knowledge/docs_legacy/architecture/` with a conformance status
...
**Note:** Operational runbooks are written by SRE. See SRE output at `brain/knowledge/docs_legacy/runbooks/`. DevOps ensures alerting configs link to the appropriate runbook paths.
...
- **Runbook links** — Every alert links to a runbook (runbooks maintained by SRE at `brain/knowledge/docs_legacy/runbooks/`)
...
| Writing operational runbooks | Runbooks belong to SRE at brain/knowledge/docs_legacy/runbooks/ — DevOps links alerts to runbook paths |
```
...
| Degraded | `brain/knowledge/docs_legacy/architecture/tech-stack.md`, `brain/knowledge/docs_legacy/architecture/architecture-decision-records/` | WARN — ask user for framework/auth choices via AskUserQuestion |
| Optional | `brain/knowledge/docs_legacy/architecture/system-diagrams/`, `schemas/erd.md`, branding guidelines | Continue — use sensible defaults |
...
Read BRD user stories and solution architect artifacts from the project root. Confirm framework, state management, and styling choices with the user. Produce a structured analysis in `Claude-Production-Grade-Suite/frontend-engineer/brain/knowledge/docs_legacy/` (workspace artifacts).
...
Create `Claude-Production-Grade-Suite/frontend-engineer/brain/knowledge/docs_legacy/user-flows.md`:
...
Create `Claude-Production-Grade-Suite/frontend-engineer/brain/knowledge/docs_legacy/page-inventory.md`:
...
Create `Claude-Production-Grade-Suite/frontend-engineer/brain/knowledge/docs_legacy/component-inventory.md`:
...
- `brain/knowledge/docs_legacy/architecture/tech-stack.md` — Framework, language, auth provider decisions
- `brain/knowledge/docs_legacy/architecture/system-diagrams/` — C4 container diagrams for understanding service boundaries
- `brain/knowledge/docs_legacy/architecture/architecture-decision-records/` — ADRs for auth strategy, API patterns, multi-tenancy
...
│   ├── pricing/page.tsx
│   └── brain/knowledge/docs_legacy/page.tsx
...
Document findings in `Claude-Production-Grade-Suite/frontend-engineer/brain/knowledge/docs_legacy/design-research.md` including the selected style and how it maps to the domain.
...
- `Claude-Production-Grade-Suite/frontend-engineer/brain/knowledge/docs_legacy/design-research.md` — Domain research, competitive analysis, trend findings
- `Claude-Production-Grade-Suite/frontend-engineer/brain/knowledge/docs_legacy/design-decisions.md` — Color choices with rationale, typography selection, visual hierarchy rules
...
Read Phase 3 component inventory from `Claude-Production-Grade-Suite/frontend-engineer/brain/knowledge/docs_legacy/component-inventory.md`. Read Phase 4 pages from `frontend/app/pages/` for route coverage. Read Phase 5 design decisions from `Claude-Production-Grade-Suite/frontend-engineer/brain/knowledge/docs_legacy/design-decisions.md` for visual regression context.
...
Produce `Claude-Production-Grade-Suite/frontend-engineer/brain/knowledge/docs_legacy/a11y-audit.md`.
...
Produce `frontend/lighthouserc.json` and `Claude-Production-Grade-Suite/frontend-engineer/brain/knowledge/docs_legacy/performance-budget.md`.
...
Produce `Claude-Production-Grade-Suite/frontend-engineer/brain/knowledge/docs_legacy/browser-support.md`.
...
- `Claude-Production-Grade-Suite/frontend-engineer/brain/knowledge/docs_legacy/a11y-audit.md`
- `Claude-Production-Grade-Suite/frontend-engineer/brain/knowledge/docs_legacy/performance-budget.md`
- `Claude-Production-Grade-Suite/frontend-engineer/brain/knowledge/docs_legacy/browser-support.md`
...
- All project root deliverables (`services/`, `api/`, `brain/knowledge/docs_legacy/`, etc.)
...
Glob("services/**/*")
Glob("frontend/**/*")
Glob("brain/knowledge/docs_legacy/**/*")
```
...
Read `brain/knowledge/docs_legacy/architecture/`, `api/`, `schemas/`, then:
...
4. **Codebase discovery — detect greenfield vs brownfield:**
   ...
   Glob("src/**"), Glob("services/**"), Glob("frontend/**"), Glob("tests/**"), Glob("brain/knowledge/docs_legacy/**")
   ...
   **If Brownfield** → run the adaptation sequence:
   ...
   Directories found: src/, tests/, brain/knowledge/docs_legacy/, .github/workflows/
...
1. **Count services** — Read `brain/knowledge/docs_legacy/architecture/` service list or `api/` specs. For each service, create a subtask under T3a.
...
```python
Agent(
  prompt="You are the Software Engineer. Implement the {service_name} service. Read architecture at brain/knowledge/docs_legacy/architecture/ and API contract at api/openapi/{service}.yaml. Follow skills/software-engineer/phases/02-service-implementation.md. Write output to services/{service_name}/.",
  subagent_type="general-purpose",
  mode="bypassPermissions",
  run_in_background=True
)
...
| T2: Architect | `product-manager/BRD/` | `api/`, `schemas/`, `brain/knowledge/docs_legacy/architecture/` | `solution-architect/` |
| T3a: Backend | `api/`, `schemas/`, `brain/knowledge/docs_legacy/architecture/` | `services/`, `libs/shared/` | `software-engineer/` |
| T3b: Frontend | `api/`, `product-manager/BRD/` | `frontend/` | `frontend-engineer/` |
| T4: DevOps | `services/`, `brain/knowledge/docs_legacy/architecture/` | Dockerfiles at root | `devops/containers/` |
...
| T9: SRE | All prior outputs | `brain/knowledge/docs_legacy/runbooks/` | `sre/` |
| T10: Data Sci | Implementation (LLM usage) | — | `data-scientist/` |
| T11: Tech Writer | ALL workspace + project | `brain/knowledge/docs_legacy/` | `technical-writer/` |
...
|-----------|-------------------|
| **DEFINE → BUILD** | `product-manager/BRD/brd.md`, `solution-architect/system-design.md`, `brain/knowledge/docs_legacy/architecture/adr/*.md` (list), `api/openapi/*.yaml` (list), `.orchestrator/settings.md`, `.orchestrator/receipts/T1-*.json`, `.orchestrator/receipts/T2-*.json` |
...
| DevOps writing runbooks | sre writes runbooks to brain/knowledge/docs_legacy/runbooks/ |
...
- `brain/knowledge/docs_legacy/architecture/adr/*.md` (Glob to list, Read key ADRs)
...
Read architecture from: api/, schemas/, brain/knowledge/docs_legacy/architecture/
...
Read architecture from: brain/knowledge/docs_legacy/architecture/
Read .production-grade.yaml for paths and preferences.
...
- `paths.adrs` → ADR location (default: `brain/knowledge/docs_legacy/architecture/architecture-decision-records/`)
- `paths.architecture_docs` → Architecture docs (default: `brain/knowledge/docs_legacy/architecture/`)
...
5. Write deliverables to **project root**: `api/`, `schemas/`, `brain/knowledge/docs_legacy/architecture/`
...
2. **Re-anchor:** Re-read from disk before transitioning:
   - `Claude-Production-Grade-Suite/product-manager/BRD/brd.md`
   - `Claude-Production-Grade-Suite/solution-architect/system-design.md`
   - `brain/knowledge/docs_legacy/architecture/adr/*.md` (list files)
   - `api/openapi/*.yaml` (list files)
   - `.orchestrator/settings.md`
3. Verify architecture outputs exist at project root (`api/`, `schemas/`, `brain/knowledge/docs_legacy/architecture/`)
...
- `Claude-Production-Grade-Suite/solution-architect/system-design.md`
- `brain/knowledge/docs_legacy/architecture/adr/*.md` (Glob to list)
...
Read architecture: brain/knowledge/docs_legacy/architecture/, api/
...
Use the Skill tool to invoke 'production-grade:devops' to load your complete methodology and follow it.
Read architecture: brain/knowledge/docs_legacy/architecture/
Read implementation: services/, frontend/
Read .production-grade.yaml for paths and preferences.
Read protocols from: Claude-Production-Grade-Suite/.protocols/
...
DO NOT write runbooks — SRE writes runbooks to brain/knowledge/docs_legacy/runbooks/.
...
Write runbooks to project root: brain/knowledge/docs_legacy/runbooks/
...
- `brain/knowledge/docs_legacy/architecture/` listing
...
Read all project deliverables: api/, services/, frontend/, infrastructure/, tests/, brain/knowledge/docs_legacy/.
Read protocols from: Claude-Production-Grade-Suite/.protocols/
Read .production-grade.yaml for paths and preferences.
...
Write docs to project root: brain/knowledge/docs_legacy/
...
- **`api/`, `schemas/`, `brain/knowledge/docs_legacy/architecture/`** — API contracts (OpenAPI/AsyncAPI specs), data models, sequence diagrams
...
- `schemas/` data models and `brain/knowledge/docs_legacy/architecture/` sequence diagrams
...
- `brain/knowledge/docs_legacy/architecture/` infrastructure requirements (which DBs, caches, brokers)
- The test plan from Phase 1
...
**Inputs to read:**
- `brain/knowledge/docs_legacy/architecture/` NFRs (latency targets, throughput requirements, SLOs)
...
| Degraded | `brain/knowledge/docs_legacy/architecture/`, `schemas/` | WARN — proceed with code-only analysis, flag reduced scope |
...
- Architecture docs -- `brain/knowledge/docs_legacy/architecture/` (ADRs, system diagrams, data flow)
...
4. **Skill type?** — Options: Technique (steps to follow), Pattern (mental model), Reference (brain/knowledge/docs_legacy/API guide), Workflow (multi-phase process)
...
**Identity:** You are the Software Engineer. Your role is to read the Solution Architect's output (`api/`, `schemas/`, `brain/knowledge/docs_legacy/architecture/`) and generate fully working, production-grade service code with business logic, API handlers, data access layers, middleware, and integration patterns.
...
| Category | Inputs | Behavior if Missing |
|----------|--------|-------------------|
| Critical | `api/openapi/*.yaml` or `api/grpc/*.proto`, `schemas/erd.md`, `brain/knowledge/docs_legacy/architecture/tech-stack.md` | STOP — cannot implement without API contracts, data models, and tech stack |
| Degraded | `brain/knowledge/docs_legacy/architecture/architecture-decision-records/`, `schemas/migrations/*.sql` | WARN — proceed with reasonable defaults, flag assumptions |
...
Product Manager          Solution Architect          Software Engineer          QA Engineer
    (BRD/PRD)     -->    (api/, schemas/,         -->  (services/, libs/,    -->  (tests/)
                          brain/knowledge/docs_legacy/architecture/)           scripts/)
```

This skill reads from `api/`, `schemas/`, and `brain/knowledge/docs_legacy/architecture/` and produces deliverables at project root (`services/`, `libs/`, `scripts/`, etc.) with workspace artifacts in `Claude-Production-Grade-Suite/software-engineer/`. It does NOT redesign the architecture or change API contracts — it implements them faithfully.
...
|-------|------|-----------------|
| Tech stack | `brain/knowledge/docs_legacy/architecture/tech-stack.md` | Language, framework, database, cache, message broker |
...
| ADRs | `brain/knowledge/docs_legacy/architecture/architecture-decision-records/` | Architecture pattern, communication patterns, data strategy, auth, multi-tenancy |
...
| `brain/knowledge/docs_legacy/architecture/tech-stack.md` | Language, framework, libraries |
| `brain/knowledge/docs_legacy/architecture/architecture-decision-records/` | Patterns, trade-offs |
| `services/` (scaffolded) | Service names, structure |

**Do NOT modify architecture files** (`api/`, `schemas/`, `brain/knowledge/docs_legacy/architecture/`). If an API contract needs changes, flag it to the user — do not unilaterally alter the architect's decisions.
```
...
Generates architecture deliverables at the project root (`api/`, `schemas/`, `brain/knowledge/docs_legacy/architecture/`, project scaffold) with workspace artifacts in `Claude-Production-Grade-Suite/solution-architect/`.

## Config Paths

Read `.production-grade.yaml` at startup. Use these overrides if defined:
- `paths.api_contracts` — default: `api/`
- `paths.adrs` — default: `brain/knowledge/docs_legacy/architecture/architecture-decision-records/`
- `paths.architecture_docs` — default: `brain/knowledge/docs_legacy/architecture/`
- `paths.erd` — default: `schemas/erd.md`
- `paths.migrations` — default: `schemas/migrations/`
- `paths.tech_stack` — default: `brain/knowledge/docs_legacy/architecture/tech-stack.md`

Deliverables go to the **project root** (`api/`, `schemas/`, `brain/knowledge/docs_legacy/architecture/`). Workspace artifacts go to `Claude-Production-Grade-Suite/solution-architect/`.
...
Generate architecture documents in `brain/knowledge/docs_legacy/architecture/` (or `paths.architecture_docs` from config):
...
Generate `brain/knowledge/docs_legacy/architecture/tech-stack.md` (or `paths.tech_stack` from config):
...
```
brain/knowledge/docs_legacy/architecture/
│   ├── architecture-decision-records/
...
Agent(prompt="Define incident management procedures following Phase 4. Write to sre/incidents/ and brain/knowledge/docs_legacy/runbooks/.", ...)
...
```
brain/knowledge/docs_legacy/runbooks/<service-name>/
...
For each service identified in the architecture, generate a directory under `brain/knowledge/docs_legacy/runbooks/<service-name>/` at the project root. Each runbook MUST follow this template:
...
- **Match existing doc style** — if they use JSDoc, use JSDoc. If they have a brain/knowledge/docs_legacy/ site, add to it
...
| `brain/knowledge/docs_legacy/architecture/` | Critical | Architect | Service boundaries, technology choices, data flow, decision rationale |
...
| `brain/knowledge/docs_legacy/runbooks/`, `Claude-Production-Grade-Suite/sre/` | Optional | SRE | Runbooks, incident procedures, SLO definitions, DR playbooks |
...
```python
Agent(prompt="Generate API reference documentation following Phase 2. Read OpenAPI specs from api/. Write to brain/knowledge/docs_legacy/api-reference/.", ...)
Agent(prompt="Generate developer guides following Phase 3. Read architecture and source code. Write to brain/knowledge/docs_legacy/getting-started/, brain/knowledge/docs_legacy/guides/, brain/knowledge/docs_legacy/operations/.", ...)
```
...
- [ ] Runbook index links to `brain/knowledge/docs_legacy/runbooks/` (single source of truth)
...
| Architecture docs | `brain/knowledge/docs_legacy/architecture/` | Service boundaries, ADRs, tech stack, data flow |
...
| SRE artifacts | `brain/knowledge/docs_legacy/runbooks/`, `Claude-Production-Grade-Suite/sre/` | Runbooks, incident procedures, SLO definitions, DR playbooks |
...
| brain/knowledge/docs_legacy/architecture/tech-stack.md | narrative | high | developer | Stack rationale |
| ... | ... | ... | ... | ... |
```
...
Generate `brain/knowledge/docs_legacy/api-reference/authentication.md`:
...
Generate `brain/knowledge/docs_legacy/api-reference/endpoints/<resource-name>.md` — one file per API resource.
...
Generate `brain/knowledge/docs_legacy/api-reference/error-codes.md`:
...
Generate `brain/knowledge/docs_legacy/api-reference/rate-limiting.md`:
...
Generate `brain/knowledge/docs_legacy/api-reference/webhooks.md` (if AsyncAPI specs or webhook implementation exists):
...
Generate artifacts in `brain/knowledge/docs_legacy/api-reference/generated/`:
...
| Artifact | Path |
|----------|------|
| Authentication guide | `brain/knowledge/docs_legacy/api-reference/authentication.md` |
| Endpoint pages | `brain/knowledge/docs_legacy/api-reference/endpoints/<resource>.md` (one per resource) |
| Error codes reference | `brain/knowledge/docs_legacy/api-reference/error-codes.md` |
| Rate limiting guide | `brain/knowledge/docs_legacy/api-reference/rate-limiting.md` |
| Webhooks guide | `brain/knowledge/docs_legacy/api-reference/webhooks.md` |
| OpenAPI JSON | `brain/knowledge/docs_legacy/api-reference/generated/openapi.json` |
| Redoc HTML | `brain/knowledge/docs_legacy/api-reference/generated/openapi.html` |
...
| Architecture docs | `brain/knowledge/docs_legacy/architecture/` | ADRs, service map, tech stack |
...
Generate `brain/knowledge/docs_legacy/getting-started/quickstart.md`:
...
Generate `brain/knowledge/docs_legacy/getting-started/local-development.md`:
...
Generate `brain/knowledge/docs_legacy/guides/contributing.md`:
...
Generate `brain/knowledge/docs_legacy/guides/testing-guide.md`:
...
Generate `brain/knowledge/docs_legacy/architecture/overview.md`:
...
Generate `brain/knowledge/docs_legacy/getting-started/installation.md` (includes env var reference):
...
Generate `brain/knowledge/docs_legacy/operations/deployment.md`:
...
Generate `brain/knowledge/docs_legacy/guides/coding-conventions.md`:
...
| Artifact | Path |
|----------|------|
| Quickstart guide | `brain/knowledge/docs_legacy/getting-started/quickstart.md` |
| Local development guide | `brain/knowledge/docs_legacy/getting-started/local-development.md` |
| Installation and env vars | `brain/knowledge/docs_legacy/getting-started/installation.md` |
| Contributing guide | `brain/knowledge/docs_legacy/guides/contributing.md` |
| Testing guide | `brain/knowledge/docs_legacy/guides/testing-guide.md` |
| Coding conventions | `brain/knowledge/docs_legacy/guides/coding-conventions.md` |
| Architecture overview | `brain/knowledge/docs_legacy/architecture/overview.md` |
| Service map | `brain/knowledge/docs_legacy/architecture/service-map.md` |
| ADR summaries | `brain/knowledge/docs_legacy/architecture/decisions/<NNN-title>.md` |
| Deployment guide | `brain/knowledge/docs_legacy/operations/deployment.md` |
...
| Input | Path | What to Extract |
|-------|------|-----------------|
| All documentation | `brain/knowledge/docs_legacy/` | Pages to organize into sidebar categories |
...
Generate the scaffold in `brain/knowledge/docs_legacy/docusaurus/`:
...
Generate `brain/knowledge/docs_legacy/docusaurus/docusaurus.config.js`:
...
Generate `brain/knowledge/docs_legacy/docusaurus/sidebars.js` matching the documentation sitemap from Phase 1:
  ...
  docs: [
    ...
    },
    {
      ...
      items: ['architecture/overview', 'architecture/service-map',
        { type: 'category', label: 'Design Decisions', items: [/* populated from brain/knowledge/docs_legacy/architecture/decisions/ */] },
      ],
    },
    {
      type: 'category', label: 'API Reference',
      items: ['api-reference/authentication',
        { type: 'category', label: 'Endpoints', items: [/* populated from brain/knowledge/docs_legacy/api-reference/endpoints/ */] },
...
1. **GitHub Pages** (default) — Generate `.github/workflows/docs-build.yml`:
   - Trigger on push to main (paths: `brain/knowledge/docs_legacy/**`)
   - Trigger on PR to main (paths: `brain/knowledge/docs_legacy/**`) for build verification
...
2. **Vercel** (alternative) — Generate `brain/knowledge/docs_legacy/docusaurus/vercel.json`:
   ```json
   {
...
3. **Netlify** (alternative) — Generate `brain/knowledge/docs_legacy/docusaurus/netlify.toml`:
   ```toml
   [build]
...
| Artifact | Path |
|----------|------|
| Docusaurus config | `brain/knowledge/docs_legacy/docusaurus/docusaurus.config.js` |
| Sidebar config | `brain/knowledge/docs_legacy/docusaurus/sidebars.js` |
| Package manifest | `brain/knowledge/docs_legacy/docusaurus/package.json` |
| Babel config | `brain/knowledge/docs_legacy/docusaurus/babel.config.js` |
| Custom CSS | `brain/knowledge/docs_legacy/docusaurus/src/css/custom.css` |
| Landing page | `brain/knowledge/docs_legacy/docusaurus/src/pages/index.js` |
| CI pipeline | `.github/workflows/docs-build.yml` |
| Changelog template | `CHANGELOG.md` |
| Deployment config | `brain/knowledge/docs_legacy/docusaurus/vercel.json` or `netlify.toml` (per user choice) |
...
- `npm run build` in `brain/knowledge/docs_legacy/docusaurus/` produces zero errors
...
| Documentation (brain/knowledge/docs_legacy/) | **technical-writer** | — |
...
Glob("brain/knowledge/docs_legacy/architecture/**/*.md")
...
arch_path = config.paths.architecture_docs || "brain/knowledge/docs_legacy/architecture/"
```
```
...
paths:
  ...
  architecture_docs: "brain/knowledge/docs_legacy/architecture/"
  adrs: "brain/knowledge/docs_legacy/architecture/architecture-decision-records/"
  tech_stack: "brain/knowledge/docs_legacy/architecture/tech-stack.md"
  system_diagrams: "brain/knowledge/docs_legacy/architecture/system-diagrams/"
  erd: "schemas/erd.md"
  ...
  docs: "brain/knowledge/docs_legacy/"
  runbooks: "brain/knowledge/docs_legacy/runbooks/"
