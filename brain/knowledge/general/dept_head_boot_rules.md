---
id: dept-head-boot-rules
type: document
owner: OHD
tags: [auto-healed]
healed_at: 2026-04-02T20:19:18.255539
---
...
## BOOT SEQUENCE — EVERY DEPT HEAD AGENT
...
BOOT-01: LOAD YOUR DEPT MEMORY FIRST
  ...
  If file doesn't exist → alert library-manager-agent to build it.

BOOT-02: CHECK YOUR KNOWLEDGE FEED
  Load: brain/corp/knowledge_feeds/<your_dept>/new_knowledge.md
  Any item with OD&L enrichment = YES → forward to training-agent (OD&L).
  Any ACTIONABLE item → schedule action this cycle.
  Any RULE change → apply to your dept rules.md + notify COO.
...
BOOT-04: READ YOUR DAILY BRIEF QUEUE
  ...
  Items marked URGENT → handle before any other task.
...
BOOT-06: KNOWLEDGE SHARING IS MANDATORY
  ...
  → Write to brain/corp/knowledge_feeds/<your_dept>/outbound_knowledge.md
  → knowledge-tagger-agent (Asset Library) will distribute it.
...
1. For each entry, tag your response:
   - [READ] — reviewed, no action needed
   - [ACTION] — creates a task this cycle  
   - [TRAIN] — agent enrichment needed → forward to OD&L
   - [RULE] — requires rule update → schedule review with COO
