...
owner: OHD
tags: [auto-healed]
healed_at: 2026-04-02T20:31:20.366502
---

# Workflow: system-repair
# Owner: system-repair-agent | Dept 32 (System Integrity)
# Trigger: "audit system" | "fix system" | "scan and fix" | "kiá»ƒm tra há»‡ thá»‘ng" | boot missing files | CI/CD encoding fail
...
| Trigger | Run Type |
|---------|----------|
| "audit system" / "kiá»ƒm tra há»‡ thá»‘ng" | Full repair |
| "fix system" / "sá»­a lá»—i há»‡ thá»‘ng" | Full repair |
...
**Check:**
- Any uncommitted changes? â†’ Log them, proceed carefully
- Any stash entries? â†’ Note and leave untouched
- Branch: confirm we're on working branch (not main)

---

## Phase 1 â€” Stale Naming Scan (Module 3)
...
**Action:** Log all occurrences â†’ Auto-replace clear matches â†’ Skip excluded paths
**Output:** `stale_naming_report` in REPAIR_RECEIPT

---

## Phase 2 â€” Encoding Fix (Module 1)
...
## Phase 3 â€” MCP Plugin Health (Module 4)
...
## Phase 4 â€” Skill Registry Sync (Module 5)

**Tool:** Python
**Scope:** `ecosystem/skills/` vs `brain/registry/SKILL_REGISTRY.json`

```python
import os, json
root = os.environ.get('OMNICLAW_ROOT', os.environ.get('AOS_ROOT', '.'))
skills_dir = os.path.join(root, 'ecosystem/skills')
registry_path = os.path.join(root, 'brain/registry/SKILL_REGISTRY.json')
...
**Action:** Add missing skills to registry â†’ Flag broken paths for manual review
**Output:** `registry_synced` count in REPAIR_RECEIPT

---

## Phase 5 â€” Path Validation (Module 2)

**Tool:** Python regex + `os.path.exists()`
**Scope:** `.mcp.json`, `*.bat`, `*.ps1`, CI/CD YAMLs, `server.js`

**Action:** Report all broken paths â†’ Confirm with operator before fixing
**Output:** `broken_paths_found` in REPAIR_RECEIPT

---

## Phase 6 â€” JSON/YAML Syntax Validation (Module 6)

**Tool:** Python json/yaml
**Scope:** All `.json`, `.yaml`, `.yml` (excluding `.git`, `node_modules`, `venv`)

**Action:** Report all parse errors â†’ Manual intervention required
**Output:** `json_yaml_errors` count in REPAIR_RECEIPT

---

## Phase 7 â€” Write REPAIR_RECEIPT
...
## Phase 8 â€” Update Blackboard & Commit

```bash
# Update blackboard.json â†’ system_status: "healthy" or "needs_attention"
# Then commit:
git add -A
...
| 2-strike failure on same task | `handoff_trigger: BLOCKED` â†’ report |

---

## References

- **Agent definition:** `ecosystem/workforce/agents/system-repair-agent.md`
- **Skill toolkit:** `ecosystem/skills/system_autofix/SKILL.md`
- **Knowledge base:** `brain/knowledge/notes/KI-SYSTEM-REPAIR-01.md`
- **Department:** `ecosystem/workforce/departments/system_integrity.yaml`
