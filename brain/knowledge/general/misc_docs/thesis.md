...
owner: OHD
tags: [auto-healed]
healed_at: 2026-04-02T20:31:22.006559
---
...
35. **Corp Mode = Structure:** Virtual AI company with C-suite agents running autonomous departments via daily brief â†’ KPI â†’ retro cycles (Corp Orchestrator Protocol).
...
38. **HITL = Safety Gate:** Human-in-the-loop approval checkpoints before high-stakes autonomous actions â€” no agent bypasses the CEO gate (HITL Gateway Protocol).
39. **Economy = Efficiency:** Route each request to the cheapest capable LLM â€” balancing quality, speed, and token cost (LLM Router Protocol).
40. **Agile = Velocity:** Sprint-based delivery with structured BMAD planning â€” from brief to backlog to shipped feature (BMAD Protocol).

### ðŸ“ The LLM Triangle (Core Engineering)
- **SOP (Standard Operating Procedure):** Our `workflows/` and `corp/prompts/runes/`.
- **Engineering:** Our `skills/`, `subagents/mq/`, and `telemetry/`.
- **Context:** Our `brain/memory/`, `brain/memory/`, and `brain/knowledge/`.

## 2. Shared Context & Infrastructure

- **`brain/memory/`** â€” Global strategies, thesis, governance, agent roster
- **`subagents/mq/<dept>_brief.md`** â€” The "Nervous System" for task passing between agents
- **`corp/prompts/runes/`** â€” Standardized ritual prompts and SOP templates
- **`telemetry/receipts/`** â€” Cryptographically-auditable task receipts (JSON, per step)
- **`ecosystem/skills/`** â€” 45+ loaded skill modules, queried via `SKILL_REGISTRY.json`
- **`plugins/`** â€” managed via registry.json (7 tier1, 7 tier2) (firecrawl, LightRAG, crewai, zeroleaks, etc.)
- **`brain/memory/report_formats.md`** â€” Standard output formats all agents must use when reporting to CEO (loaded at boot Step 6)
...
| `brain/memory/` | GOVERNANCE, AGENTS, SOUL, THESIS | loaded at boot |
...
A recursive history of growth â€” multi-layer, multi-timeframe.
...
- Tier 0 files are **read-only for all agents** â€” CEO modifies only
...
| Audience | Language |
|----------|----------|
| CEO output (Antigravity â†’ Human) | **Vietnamese** |
...
| Rule | Policy |
|------|--------|
| **2-Strike Rule** | Claude Code: FAIL 1 â†’ try different approach â†’ FAIL 2 â†’ BLOCKED, stop and report |
| **Receipt-per-Action** | Every autonomous action must produce a JSON receipt in `telemetry/receipts/` |
| **Boot Fallback** | Missing boot file â†’ log warning, skip step, report to CEO. Never assume defaults |
| **Security Gate** | New plugin/tool/integration â†’ Strix/GRC scan FIRST before activation |
...
All CEO-facing reports must follow formats in `brain/memory/report_formats.md`.
