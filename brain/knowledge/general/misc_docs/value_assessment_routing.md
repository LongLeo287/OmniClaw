---
id: value-assessment-routing
type: document
owner: OHD
tags: [auto-healed]
healed_at: 2026-04-02T20:32:40.351424
---
...
## VALUE_TYPE TAXONOMY — Full 9 Categories
...
| `RULE_POLICY` | Governance rule, SOP, policy, or constraint definition | Defines MUST/SHOULD/NEVER behavior for agents | rule-builder-agent → relevant dept |
| `AGENT_DEFINITION` | New agent spec or prompt template | Defines agent role, responsibilities, skills | OD&L → dept-builder-agent |
| `DATA_ASSET` | Structured data, dataset, config file, template | JSON/CSV/YAML/XLSX — not code, not knowledge | Asset Library → asset-tracker |

---

## VALUE_TYPE SIGNALS — HOW TO DETECT
...
## ROUTING MATRIX — WHERE EACH VALUE_TYPE GOES
...
| `RULE_POLICY` | `corp/rules/` or `corp/departments/<dept>/rules.md` | rule-builder-agent drafts routing | rule-builder-agent (Registry) → COO |
| `AGENT_DEFINITION` | `corp/departments/<dept>/rules.md` (new agent spec) | dept-builder-agent builds full package | OD&L → dept-builder-agent |
| `DATA_ASSET` | `assets/data/<name>` | Catalog + metadata | asset-tracker-agent (Asset Library) |

---

## CONFLICT RESOLUTION — TRAIN, DON'T OVERWRITE
...
Does destination already have this resource?
    NO  → CREATE NEW (normal routing)
    YES → CONFLICT RESOLUTION FLOW:
              1. COMPARE versions (new vs existing)
              2. ASSESS delta: what is genuinely new/better?
              3. If new has improvements → UPGRADE (TYPE 2 Enrichment via training-agent)
              4. If existing is already better → DISCARD new, log reason
              5. If both have unique value → MERGE selectively
...
|-----------|-----------------|------------|
| `SKILL` | Skill already in SKILL_REGISTRY | Compare capability scope → upgrade if new skill covers more cases or adds new tools → training-agent patches agent rules.md, does NOT replace SKILL.md |
| `KNOWLEDGE` | knowledge/ file already exists | Append new sections or update stale facts → knowledge-curator merges delta, marks source |
| `WORKFLOW` | workflow file already exists | Compare steps — if new has clearer steps or new edge cases → archivist-agent appends as revision note or replaces specific section, not entire file |
| `PLUGIN` | plugin already cataloged | Check version number — upgrade only if new version > existing and changelog confirms improvement |
| `MCP_SERVER` | MCP server already running | Do NOT replace — alert sysadmin for review. New version = staging test first, then swap |
| `TOOL_SCRIPT` | script already in scripts/ | Compare logic — if genuinely better: rename old as `<name>.v1.bak`, deploy new |
| `RULE_POLICY` | rule already exists in brain/corp/rules/ | NEVER auto-overwrite. rule-builder-agent flags conflict → COO decision only |
...
Primary: SKILL
Destinations:
  → ecosystem/skills/new_skill/ (skill-creator-agent)
  → knowledge/ai_techniques/ (knowledge-curator-agent)
```
...
3. **CHECK FIRST: Does resource already exist at destination?**
   - NO → proceed with normal routing (CREATE)
   - YES → enter CONFLICT RESOLUTION FLOW (see section above)
   - NEVER route without this check
4. On CREATE: move to destination, write delivery receipt
5. On CONFLICT: write CONFLICT DETECTED output → route to training-agent, not destination
6. Update ticket to INGESTED (CREATE) or ENRICHMENT_PENDING (CONFLICT)
7. All destinations confirmed before ticket close

If VALUE_TYPE = UNKNOWN:
  → Hold content in QUARANTINE/vetted/ 
  → Alert intake-chief-agent for manual VALUE_TYPE assessment
  → SLA: must be resolved within 2 corp cycles

---

## EXAMPLES — Real Content Through the Pipeline
...
VALUE_TYPE assessment:
  ✓ KNOWLEDGE (excellent research README)
  ✓ SKILL (reusable agent patterns in Python)
  ✗ PLUGIN (no CLI interface)
  ✗ MCP_SERVER (no MCP protocol)
ROUTE TO:
  → skills/ (skill-creator-agent wraps patterns)
  → knowledge/ai_frameworks/ (knowledge-curator indexes)
```
...
VALUE_TYPE assessment:
  ✗ KNOWLEDGE (partial — too policy-specific)
  ✗ SKILL
  ✓ RULE_POLICY (defines MUST/PROHIBITED behaviors for AI systems)
ROUTE TO:
  → rule-builder-agent reviews for conflict
  → draft to brain/corp/rules/ai_governance_external.md
  → COO notified for approval
```
...
VALUE_TYPE assessment:
  ...
  ✓ WORKFLOW (contains step-by-step tutorial)
  ✗ MCP_SERVER (article about MCP, not an MCP impl)
ROUTE TO:
  → knowledge/mcp_development/ (knowledge-curator)
  → workflows/mcp-server-build-guide.md (archivist-agent)
```
...
VALUE_TYPE assessment:
  ✗ SKILL
  ✗ PLUGIN (no integration interface)
  ✓ TOOL_SCRIPT (standalone automation script)
ROUTE TO:
  → scripts/monitor-agent-performance.ps1 (sysadmin-agent catalogs)
