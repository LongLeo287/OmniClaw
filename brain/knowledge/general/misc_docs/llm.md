...
owner: OHD
tags: [auto-healed]
healed_at: 2026-04-02T20:29:05.319867
---
...
# Initialize client
client = MemoryClient(API_KEY='[REDACTED_API_KEY]')
...
const client = new MemoryClient({ API_KEY='[REDACTED_API_KEY]' });
...
const memory = new Memory({
  embedder: { provider: 'openai', config: { API_KEY='[REDACTED_API_KEY]' } },
  vectorStore: { provider: 'memory', config: { dimension: 1536 } },
  llm: { provider: 'openai', config: { API_KEY='[REDACTED_API_KEY]' } }
});
...
```python
client = MemoryClient(
    API_KEY='[REDACTED_API_KEY]',  # or set MEM0_API_KEY env var
...
const memory = new Memory({
  embedder: {
    provider: 'openai',
    config: { API_KEY='[REDACTED_API_KEY]' }
  },
  vectorStore: {
    provider: 'qdrant',
...
class SupportBot:
    def __init__(self):
        self.memory = MemoryClient(API_KEY='[REDACTED_API_KEY]')
...
app = FastAPI()
memory_client = MemoryClient(API_KEY='[REDACTED_API_KEY]')
