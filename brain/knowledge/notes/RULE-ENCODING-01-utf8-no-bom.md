...
- JSON: BOM breaks `json.loads()` with JSONDecodeError (occurred in `system/infra/mcp/config.json`)
...
pwsh system/ops/scripts/fix_encoding_bom.ps1
# Dry-run (no changes):
pwsh system/ops/scripts/fix_encoding_bom.ps1 -DryRun
```
