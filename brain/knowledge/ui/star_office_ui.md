...
```
→ Đọc từ thư mục `memory/` nằm ngoài gốc project. **Khớp với OmniClaw memory pattern.**
...
## SKILL.md — OpenClaw Convention (Tương đồng OmniClaw)
...
**Đây chính là pattern OmniClaw Skill đang dùng, xác nhận chúng ta đi đúng hướng.**
...
## Áp Dụng Vào OmniClaw

| Concept | Cách áp dụng |
|---------|--------------|
| **State API Pattern** | Thêm `/set_state` endpoint vào OmniClaw để track Agent mode |
| **Memory→Daily Card** | Đọc `memory/daily/*.md` → hiển thị "Yesterday" summary khi boot |
| **PII Redaction** | Áp dụng `sanitize_content()` vào OmniClaw khi render memory cho user |
...
| **SKILL.md Doc Pattern** | Đã confirm convention, OmniClaw skills đang đúng hướng |
