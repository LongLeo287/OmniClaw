...
startCommand:
  ...
  configSchema:
    ...
    properties:
      apiKey:
        type: string
        description: "Xquik API key from https://xquik.com/account-manager"
  url: "https://xquik.com/mcp"
...
```json
{
  ...
  "metadata": {
    "description": "X (Twitter) data extraction & automation skill for AI coding agents. 120 endpoints, 2 MCP tools, reads from $0.00015/call.",
    "version": "2.0.0"
  },
  "plugins": [
    {
      "name": "x-twitter-scraper",
      "source": "./",
      "description": "120 REST API endpoints, 2 MCP tools, 23 extraction types. Tweet search, user lookup, follower extraction, engagement metrics, giveaway draws, trending topics, write actions, credits, pay-per-use (MPP), and Telegram integrations. Reads from $0.00015/call — 33x cheaper than the official X API.",
      "version": "2.0.0",
      "author": {
        "name": "Xquik"
      },
...
```json
{
  "name": "x-twitter-scraper",
  "version": "2.0.0",
  "description": "X (Twitter) real-time data platform skill with REST API (120 endpoints), MCP server (2 tools) & webhooks. Tweet search, user lookup, user tweets/likes/media, favoriters, mutual followers, bookmarks, timeline, DM history, follower extraction, engagement metrics, media download, 23 extraction tools, account monitoring, trending topics, giveaway draws, write actions, credits, Telegram integrations, flow automations, support tickets, and pay-per-use (MPP). Reads from $0.00015/call.",
  "author": {
    "name": "Xquik",
  ...
  "userConfig": {
    "api_key": {
      "description": "Xquik API key (get one at xquik.com/account-manager)",
      "sensitive": true
    }
  }
}
...
### File: .well-known\glama.json
```json
{"$schema":"https://glama.ai/mcp/schemas/connector.json","maintainers":[{"email":"support@xquik.com"}]}

```

### File: commands\post.md
```md
---
