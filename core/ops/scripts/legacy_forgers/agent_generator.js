...
const AGENTS_DIR = 'ecosystem/workforce/agents';
...
AGENTS.forEach(agent => {
  ...
  const yamlFile = path.join(dir, 'agent.yaml');
  
  // Already exists with SKILL.md (like antigravity) — add agent.yaml beside it
  fs.mkdirSync(promptsDir, { recursive: true });

  if (!fs.existsSync(yamlFile)) {
    const yaml = `name: ${agent.id}
version: 1.0.0
...
created: 2026-03-26
source: AI OS V3.1 Agent Generator
`;
    fs.writeFileSync(yamlFile, yaml);
    console.log('[OK] Created agent.yaml:', agent.id);
  } else {
    console.log('[SKIP] agent.yaml exists:', agent.id);
  }
