...
while _curr != _curr.parent and not (_curr / "system" / "ops").is_dir():
    _curr = _curr.parent
ROOT = _curr if (_curr / "system" / "ops").is_dir() else Path(os.environ.get("OMNICLAW_ROOT", str(Path(__file__).resolve().parents[6])))
...
def evaluate_and_route(repo_name: str, md_filepath: str):
    ...
    if not path.exists():
        log(f"[ERROR] Not found file {md_filepath}")
        return
    ...
    if is_agent and ("python" in content or "framework" in content):
        ...
        if result.returncode == 0:
            ...
            log(f"[{repo_name}] Error Scaffold: {result.stderr.strip()}")
