...
Usage:
  python ops/omniclaw.py corp start           # Phase 0+1: Boot + CEO Brief
  python ops/omniclaw.py corp dispatch <dept> # Phase 2-3: Dispatch tasks
  python ops/omniclaw.py corp retro           # Phase 5-7: Synthesis + Retro + HUD
  python ops/omniclaw.py status               # Show cycle status
  python ops/omniclaw.py brief                # Show CEO daily brief (Phase 1)
  python ops/omniclaw.py proposals            # List pending CEO proposals
  python ops/omniclaw.py proposals approve <id>  # Approve a proposal
  python ops/omniclaw.py dispatch <dept>      # Dispatch tasks to a department
  python ops/omniclaw.py hud update
  python ops/omniclaw.py intake <url>       # CIV pipeline: classify + ticket + receipt           # Run update_hud.ps1
  python ops/omniclaw.py rebuild-index        # Rebuild FAST_INDEX from scratch

Shortcuts: aos = python ops/omniclaw.py  (add alias to your shell profile)
"""
import json
import os
import sys
...
def cmd_corp_start():
    ...
    checks = [BB_PATH, KPI_PATH,
              ...
              os.path.join(ROOT,"brain","registry","SKILL_REGISTRY.json"),
