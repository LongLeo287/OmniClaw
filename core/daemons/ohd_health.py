...
from core.daemons.daemon_trust import authenticate_daemon, assert_write_access, abs_path, get_handoff_targets, PATHS
from core.daemons.daemon_utils import (load_oma_map, deep_scan, report_before, report_after, report_error,
...
QA_RECEIPTS_DIR = abs_path(os.path.join(PATHS.QA_RECEIPTS, "gate_qa"))
...
def _write_quality_receipt(target_path: str, quality_report: dict):
    """Persist repo quality receipts for downstream review."""
    os.makedirs(QA_RECEIPTS_DIR, exist_ok=True)
    stem = os.path.basename(target_path).replace(" ", "_")
    receipt_path = os.path.join(QA_RECEIPTS_DIR, f"{stem}_quality_gate.json")
    payload = {
        "target": target_path,
...
def execute_auto_linter(folder_path: str) -> dict:
    """
    [PHASE 2: OHD DOCTOR]
                ...
                try:
                    ...
                    if res.returncode != 0:
                        findings.append({
                            "file": fpath,
                            "type": "syntax_error",
                            "detail": res.stderr.strip().split(os.linesep)[-1],
                        })
    ...
    report = {
        ...
        "findings": findings,
        "penalties": [{"reason": "syntax_error", "points": 20, "count": len(findings)}] if findings else [],
    }
    if not findings:
        print(f"  \033[92m[OK]\033[0m [OHD] Surgery Complete: Zero structural syntax errors detected.")
    else:
        print(f"  \033[93m[WARN]\033[0m [OHD] Surgery Complete: {len(findings)} files require manual medical attention.")
    return report
    ...
    for fname in candidates:
        ...
        try:
            ...
            if os.path.isdir(fpath):
                ...
                quality_report = execute_auto_linter(fpath)
                _write_quality_receipt(fpath, quality_report)
                if quality_report["score"] < QUALITY_THRESHOLD:
                    os.makedirs(REJECTED, exist_ok=True)
                    dest = os.path.join(REJECTED, f"QA_BLOCKED_{fname}")
                    if os.path.exists(dest):
                        dest = f"{dest}_{datetime.now().strftime('%H%M%S')}"
                    shutil.move(fpath, dest)
                    print(
                        f"  \033[41m\033[97m[BLOCKED]\033[0m [OHD] Quality gate score {quality_report['score']} < {QUALITY_THRESHOLD}. "
                        f"Agent payload returned for repair."
                    )
                    log_action("QUALITY_GATE_BLOCK", fpath, f"score={quality_report['score']}")
                    results["skipped"] += 1
                    continue
