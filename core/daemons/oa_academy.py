...
def process_workforce_agents():
    ...
    for agent_name in agents:
        ...
        if not os.path.exists(id_path):
            print(f"[\033[94mOA-IN-PLACE\033[0m] Found un-assimilated Agent Workspace: agents/{agent_name}. Initiating In-Place Assimilation...")
            try:
                _assimilate_repo(agent_dir, agent_name, in_place=True)
                print(f"    \033[92m[OK]\033[0m Successfully assimilated agents/{agent_name} in-place.")
            except Exception as e:
                print(f"    \033[91m[ERR]\033[0m In-place assimilation failed for agents/{agent_name}: {e}")
