#!/usr/bin/env python3
"""
vet_media_docs.py — Media and Document Vetting Tool
Static Binary Analysis to block embedded malicious code in images (Steganography/Polyglots) and documents (Macro/JS).
"""
import os
import sys
import argparse
import zipfile

MAGIC_BYTES = {
    ".png": b"\x89PNG\r\n\x1a\n",
    ".jpg": b"\xff\xd8",
    ".jpeg": b"\xff\xd8",
    ".gif": b"GIF8",
    ".pdf": b"%PDF-",
}

def check_magic_bytes(fpath, ext) -> tuple[bool, str]:
    """Returns (ok: bool, message: str) — ALWAYS a 2-tuple."""
    if ext not in MAGIC_BYTES:
        return True, "SKIP: magic bytes not verified for this extension"
    try:
        with open(fpath, "rb") as f:
            header = f.read(8)
            expected = MAGIC_BYTES[ext]
            if not header.startswith(expected):
                return False, f"Invalid Magic Bytes for {ext}"
    except Exception as e:
        return False, f"File read error: {e}"
    return True, "PASS"

def vet_pdf(fpath):
    try:
        with open(fpath, "rb") as f:
            content = f.read()
            # Find dangerous patterns in PDF (JS, Launch, AcroForm)
            if b"/JS" in content or b"/JavaScript" in content:
                return False, "PDF contains dangerous JavaScript (/JS)"
            if b"/Launch" in content:
                return False, "PDF contains Launch execution command (/Launch)"
            if b"/AA" in content or b"/OpenAction" in content:
                # Warning OpenAction but only block if EmbeddedFiles or JS exists (simplified)
                pass
    except Exception as e:
        return False, f"Error scan PDF: {e}"
    return True, "PASS"

def vet_docx(fpath):
    # DOCX is a zip file.
    if not zipfile.is_zipfile(fpath):
        return False, "DOCX/XLSX is not a valid ZIP format"
    try:
        with zipfile.ZipFile(fpath, 'r') as z:
            namelist = z.namelist()
            for name in namelist:
                if name.endswith("vbaProject.bin"):
                    return False, "DOCX/XLSX contains potentially malicious Macro/VBA (vbaProject.bin)"
                if name.endswith(".exe") or name.endswith(".bat") or name.endswith(".vbs"):
                    return False, f"Document contains hidden executable: {name}"
    except Exception as e:
        return False, f"Error scanning ZIP structure: {e}"
    return True, "PASS"

def vet_file(fpath):
    if not os.path.exists(fpath):
        return False, "File does not exist"

    ext = os.path.splitext(fpath)[1].lower()

    # 1. Magic Bytes Check
    ok, msg = check_magic_bytes(fpath, ext)
    if not ok: return False, msg

    # 2. Format specific checks
    if ext == ".pdf":
        return vet_pdf(fpath)
    elif ext in [".docx", ".xlsx", ".pptx", ".docm", ".xlsm", ".pptm"]:
        return vet_docx(fpath)
    elif ext in [".doc", ".xls", ".ppt"]:
        return False, f"Legacy Office format ({ext}) is not supported for security reasons (OLE2 Macro Virus). Please use .docx/.xlsx"
    elif ext == ".svg":
        try:
            with open(fpath, "r", encoding="utf-8", errors="ignore") as f:
                content = f.read().lower()
                if "<script" in content or "javascript:" in content or "onload=" in content or "onerror=" in content:
                    return False, "SVG contains unauthorized XSS/JavaScript execution code"
        except Exception:
            pass

    # For images, magic byte is sufficient for basic polyglot detection
    return True, "PASS"

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Media & Document Vetting Tool")
    parser.add_argument("file", help="Path to file to scan")
    args = parser.parse_args()

    ok, reason = vet_file(args.file)
    if ok:
        print("PASS")
        sys.exit(0)
    else:
        print(f"FAIL: {reason}")
        sys.exit(1)