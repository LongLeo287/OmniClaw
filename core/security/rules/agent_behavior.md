...
6. **⚓ RE-ANCHOR**: At every phase transition, you **MUST** re-read `THESIS.md`, `.agents/tasks/task.md`, and **[ORCHESTRATION_SOP.md](ORCHESTRATION_SOP.md)**.
    - **RE-ANCHOR:** Every 30 minutes, re-read `CLAUDE.md`, `THESIS.md`, and the new **[SOUL.md](SOUL.md)**, **[AGENTS.md](AGENTS.md)**, **[WORKFLOW.md](WORKFLOW.md)**, **[ORCHESTRATION_SOP.md](ORCHESTRATION_SOP.md)**, and **[blackboard.json](../brain/memory/blackboard.json)** to ensure behavioral alignment.
...
5.  **Storage Topology (Layer 1 vs Layer 3):**
    - **Layer 1 Brainstorms (Executive-to-User):** When brainstorming with the USER to present ideas, output MUST be saved in the Agent's Native Artifact System (`C:\Users\%USERNAME%\.gemini\antigravity`) to support Rich UI rendering.
    - **Layer 3 Brainstorms (Agent-to-Agent):** When conducting autonomous multi-agent consensus or internal brainstorming, output MUST be saved to `brain/memory/corp_memory/brainstorms/SESSION_<TaskID>.md` using the provided template. NEVER push internal meetings to the UI layer.
    - All technical files (code) MUST remain in the local project directory `D:\OmniClaw\`.
