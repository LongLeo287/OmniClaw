...
1. **Technical Files:** All files (`.md`, `.rule`, `.skill`, `.plugin`, `.prompt`, `task`, `phase`, `roadmap`) MUST be written in **English**.
2. **User Communication:** Brainstorms, Reports to the user, and `.resolved` files MUST be in **English**. (Exception to Rule 1).
...
All rules are anchored in `brain/knowledge/general/GOVERNANCE.md`. 
**STRICT RULE:** Allowed to operate in the OmniClaw Ecosystem. Creating files or folders outside this ecosystem is restricted unless directed.
**STORAGE DIRECTIVE:** Observe Layer 1 vs Layer 3 Brainstorm Rules. Layer 1 UI Reports go to `.gemini\antigravity`. Layer 3 Autonomous Agent brainstorms go to `brain/memory/corp_memory/brainstorms/`.
