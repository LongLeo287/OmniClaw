...
$registry = Get-Content "$OMNICLAW_ROOT\shared-context\SKILL_REGISTRY.json" | ConvertFrom-Json
...
```powershell
$events = Get-ChildItem "$OMNICLAW_ROOT\subagents\mq\" -Filter "facts_extracted_*.json"
...
& "$OMNICLAW_ROOT\scripts\skill_loader.ps1"
```
