...
const server = new Server(
  { name: "omniclaw-workspace", version: "1.0.0" },
  { capabilities: { tools: {} } }
);
...
async function main() {
  if (process.argv.includes("--test")) {
    console.log("✅ omniclaw-workspace MCP server OK — AOS_ROOT:", AOS_ROOT);
    process.exit(0);
  }
  const transport = new StdioServerTransport();
  await server.connect(transport);
  console.error("omniclaw-workspace MCP server running on stdio");
}
