...
Quy trình:
1. Dept 10 (Strix): Quét tên chặn Malware/Game/Ransomware.
2. Dept 11 (API Crawler): Fetch GitHub Metadata & README (Delay 0.3s chống Ban).
3. Dept 01 (Golden Eval): Đọc README chấm điểm Tech stack (Agent, RAG, MCP...).
4. Dept 20 (CIV): Lọc danh sách APPROVE, tạo CIV Tickets. Ghi REFERENCE ra Catalog.

Dữ liệu đầu vào: ALL_PENDING_REPOS.txt
"""
...
def dept10_strix_check(repo_name):
    ...
    for bad in BAD_PATTERNS:
        if bad in name_l:
            return False, f"Chứa mã độc '{bad}'"
    return True, "SAFE"
...
def run_deep_eval(urls, limit=10):
    log(f"Đang phân tích tối đa {limit} repos...")
...
def process_results(results):
    log("=== XỬ LÝ KẾT QUẢ XUẤT (DEPT 20 CIV) ===")
    ...
    log(f"[+] Tạo thành công {ticket_count} CIV Tickets vào {QUARANTINE_DIR}.")
    ...
    with open(REF_CATALOG_FILE, 'a', encoding='utf-8') as f:
        ...
        for item in results['REFERENCE']:
            f.write(f"- {item['url']} (⭐ {item['stars']} | Khớp: {item['score']}đ)\n")
            
    log(f"[+] Nạp {len(results['REFERENCE'])} repo vào Thư viện tham khảo.")
...
if __name__ == '__main__':
    log("KHỞI ĐỘNG OMNICLAW DEEP EVALUATOR...")
    if not os.path.exists(INPUT_FILE):
        log(f"KhÔng tìm thấy file: {INPUT_FILE}")
        exit()
        
    with open(INPUT_FILE, 'r', encoding='utf-8') as f:
        urls = [l.strip() for l in f if l.strip()]
        
    log(f"Đã nạp {len(urls)} URLs.")
    ...
    # KHI HOÀN TẤT, DỌN DẸP CÁC REPO ĐÃ XỬ LÝ KHỎI GITHUB.TXT ĐỂ KHÔNG BỊ TRÙNG LẶP
    urls_remaining = urls[limit:]
    with open(INPUT_FILE, 'w', encoding='utf-8') as f:
        for u in urls_remaining:
            f.write(u + '\n')
    log(f"Đã dọn dẹp nội dung Github.txt. Còn lại {len(urls_remaining)} chưa phân tích.")
