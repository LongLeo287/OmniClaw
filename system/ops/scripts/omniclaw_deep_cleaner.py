...
# Core data files in vault that should never be deleted by the cleaner
PROTECTED_DATA_FILES = {
    "ALL_PENDING_REPOS.txt", "APPROVED_REPOS.txt", "EMPTY_OR_ERROR_REPOS.txt", 
    "github_repos_clean.txt", "github_repos_only.txt", "SWEEP_MASTER_REPOS.txt", 
    "master_global_urls.txt", "KEY API.txt", "missing_repos.txt", "Github.txt"
}
    ...
    for target in all_targets:
        ...
        for root, dirs, files in os.walk(target):
            ...
            for file in files:
                ...
                # Cường hóa: Bỏ qua các file dữ liệu được bảo vệ
                if fpath.name in PROTECTED_DATA_FILES:
                    continue
...
def run_deep_cleaner():
    parser = argparse.ArgumentParser(description="OmniClaw Facility Sanitation Script")
    ...
    print("="*60)
    print(f" OMNICLAW DEEP CLEANER (Khởi động bởi Dept 22 Facility Agent)")
